...
def subtract():
    try:
        ...
        if num2 > num1:
            ...
            lambda_num2 = int(((299792458 / (num2 * 1000000)) / 2)*100)/100
            points_2 = num3 / float(lambda_num2)

            result_label.config(text=f"Расстояние(М): {round(4 * lambda_gz, 3)}")
            lambda_label.config(text=f"λ: {round(lambda_gz, 3)}")
            result_points.config(text=f"Точки(поле): {round(points, 1)}; {round(points_2,3)}")
            ...
            points = num3 / round(lambda_gz / 2, 3)
            points_gz = difference_gz / num4
            lambda_num2 = int(((299792458 / (num1 * 1000000)) / 2) * 100) / 100
            points_2 = num3 / float(lambda_num2)
...
lambda_label = tk.Label(root, text="λ: ")
lambda_label.grid(row=5, column=1, columnspan=1)
